...
def projectile_helper(speed, height, angle):
    """
    A utility function that creates a projectile, calculates its trajectory,
    and prints all relevant information.
    
    Args:
        speed (float): Initial speed of the projectile in m/s
        height (float): Initial height of the projectile in m
        angle (float): Launch angle in degrees
        
    Returns:
        Graph: The graph object containing the trajectory
    """
    # Create the projectile with the given parameters
    ball = Projectile(speed, height, angle)
    
    # Print the projectile details
    print(ball)
    
    # Calculate all coordinates for the trajectory
    coordinates = ball.calculate_all_coordinates()
    
    # Create a graph object with the coordinates
    graph = Graph(coordinates)
    
    # Print the coordinates table
    print(graph.create_coordinates_table())
    
    # Print the trajectory visualization
    print(graph.create_trajectory())
    
    # Return the graph object for further use if needed
    return graph
